...
class HandMaskGenerator:
    ...
    def update(
        ...
        if self.config.enabled:
            for detection in detections:
                if detection.score < self.config.confidence_threshold:
                    continue
                points = mask_points(detection, frame_size)
                if len(points) >= 3:
                    cv2.fillConvexPoly(target, cv2.convexHull(points), 255, lineType=cv2.LINE_AA)
        ...
        if self._smoothed is None or self._size != frame_size:
            self._smoothed = target_float
            self._size = frame_size
        else:
            alpha = 1.0
            if self.config.temporal_response > 0.0:
                alpha = 1.0 - math.exp(-delta_seconds / self.config.temporal_response)
            self._smoothed += (target_float - self._smoothed) * alpha
        return np.clip(self._smoothed * 255.0, 0, 255).astype(np.uint8)[:, :, None]
